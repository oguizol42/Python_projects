...
class SecurePlant:
    ...
    def get_info(self) -> None:
        ...
        if self.__plant.height >= 0:
            ...
            print(
                f"Invalid operation attempted: height {self.__plant.height}cm "
                "[REJECTED]"
            )
            print("Security: Negative height rejected")
        if self.__plant.age < 0:
            print(
                f"Invalid operation attempted: day {self.__plant.age}day"
                " [REJECTED]"
            )
            print("Security: Negative day rejected")
            print("")
        if self.__plant.height >= 0:
            if self.__plant.age >= 0:
                print(
                    f"Current plant: {self.__plant.name}"
                    f"({self.__plant.height}cm,"
                    f"{self.__plant.age} days)"
                )
                print()
