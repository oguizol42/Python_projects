...
class Flower(Plant):
    ...
    def __init__(
        self, name: str, height: int, age: int, color: str, bloom_str: str
    ):
...
class Tree(Plant):
    ...
    def produce_shade(self) -> None:
        print(
            f"{self.name} provide {self.produce_shade_int}"
...
def init_list_flower() -> list[Flower]:
    ...
    flow = [
        Flower(name, height, age, color, bloom)
        for name, height, age, color, bloom in p_data
    ]
    return flow
...
def init_list_tree() -> list[Tree]:
    """Init List of Trees"""
    p_data = [("Oak", 500, 1825, 50, 78), ("Olivier", 700, 365000, 20000, 6)]
    tree = [
        Tree(name, height, age, trunk_diameter, produce_shade)
        for name, height, age, trunk_diameter, produce_shade in p_data
    ]
    return tree
...
def init_list_vegetable() -> list[Vegetable]:
    ...
    vegetables = [
        ...
        for name, height, age, harvest_season, nutritional_value in p_data
    ]
    return vegetables
