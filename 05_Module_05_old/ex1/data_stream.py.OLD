...
class SensorStream(DataStream):
    ...
    def get_stats(self) -> Dict[str, Union[str, int, float]]:
        """ Return stream statistics """
        if self.data_batch[0] is False:
            return ["ERROR: data is not list of 3 floats", 0]
        return ["avg temp:", self.data_batch[0]]
...
def main() -> None:
    ...
    try:   
        ...
        print(f"[temp:{management.stream_process.temp}, ", end = "")
        print(f"humidity:{management.stream_process.data_batch[1]}, ", end = "")
        print(f"pressure:{management.stream_process.data_batch[2]}")
        print(
            "Sensor analysis: "
            f"{management.result_str}, "
            f"avg temp: {management.stream_process.temp}°C"
        )
